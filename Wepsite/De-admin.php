...
<body>
	...
	<br>
		...
		<form action="De-admin.php" method="post" class="w3-form w3-card-4" style="margin-top:30px;">
      ...
      <?php
$query = "SELECT * FROM LIBRARY_ADMIN ORDER BY FIRST_NAME"; 
...
oci_execute($parseRequest);
		...
		?>
					  <tr>
						<td >
                        	<input name="checkbox1[]" type= "checkbox" class="w3-check" value="<?=$row["ADMIN_ID"]?>" >
						</td>
					<td><div align="center"><?php echo $row["FIRST_NAME"];?></div></td>
       	 			<td><div align="center"><?php echo $row["LAST_NAME"];?></div></td>
       				<td><div align="center"><?php echo $row["ADMIN_ID"];?></div></td>
                         </tr>
               <?php
		}
		...
		if(isset($_POST['submit']) && isset($_POST['checkbox1']))
			{
				...
				{
				$query3 ="DELETE FROM LIBRARY_ADMIN WHERE ADMIN_ID = '$drop'";
				$parseRequest3 = oci_parse($conn, $query3);
				oci_execute($parseRequest3);
				}
				die( '<script>window.location = "De-Admin.php";</script>');
