...
<body>
	...
	<div class="w3-topnav w3-large w3-cyan" style="color:white;">
    	<form action="De-librarian.php" method="post">
	...
	<br>
		...
		<form action="De-librarian.php" method="post" class="w3-form w3-card-4" style="margin-top:30px;">
      ...
      <?php
$query = "SELECT * FROM LIBRARY_LIBRARIAN ORDER BY FIRST_NAME"; 
...
oci_execute($parseRequest);
		...
		?>
					  <tr>
						<td >
                        	<input name="checkbox1[]" type= "checkbox" class="w3-check" value="<?=$row["LIBRARIAN_ID"]?>" >
						</td>
					<td><div align="center"><?php echo $row["FIRST_NAME"];?></div></td>
       	 			<td><div align="center"><?php echo $row["LAST_NAME"];?></div></td>
       				<td><div align="center"><?php echo $row["LIBRARIAN_ID"];?></div></td>
                         </tr>
               <?php
		}
		$ADMIN_ID = $_SESSION['FIRST_NAME'];
		if(isset($_POST['submit']) && isset($_POST['checkbox1']))
			{
				foreach( $_POST['checkbox1'] as $drop )
				{
				$query3 ="DELETE FROM LIBRARY_LIBRARIAN WHERE LIBRARIAN_ID = '$drop'";
				$parseRequest3 = oci_parse($conn, $query3);
				oci_execute($parseRequest3);
				}
				die( '<script>window.location = "De-librarian.php";</script>');
