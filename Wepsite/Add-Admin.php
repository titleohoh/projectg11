...
<div id="Add book" class="w3-container" style="text-align: center;">
				<form action="Add-Admin.php" method="post" class="w3-form w3-card-4" style="margin-top:10px">
					<h2 class="w3-container w3-cyan">Add Admin</h2>
					<p>
						<?php
			$FORM['Firstname'] = "";
			...
			?>
             ...
             <?php
			$FORM['ADMIN_ID'] = "";
			if (isset($_POST['ADMIN_ID'])) $FORM['ADMIN_ID'] = htmlspecialchars($_POST['ADMIN_ID']);
			?>
						<h3 style="margin-left:-275px;">ID</h3>
						<input type="text" name="ADMIN_ID" value="<?php echo $FORM["ADMIN_ID"];?>" style="width:300px">
						</p>
	...
	if(isset($_POST["Add"])){
		...
		{
		 	$query1 = "INSERT INTO LIBRARY_ADMIN (FIRST_NAME,LAST_NAME,ADMIN_ID)
		 			VALUES('".$_POST["Firstname"]."','".$_POST["Lastname"]."','".$_POST["ADMIN_ID"]."')";
					$parseRequest1 = oci_parse($conn, $query1);
					oci_execute($parseRequest1);
					echo"ลงทะเบียนเสร็จแล้ว";
					die( '<script>window.location = "Add-Admin.php";</script>');
